...
- Gemini wins 16 of 19 shared benchmarks and runs about twice as fast. DeepSeek costs roughly 61% less. Neither lab is trying to win the same argument.
...
Gemini 3.7 Flash isn't a new model from scratch. Its [model card describes it as algorithmic improvements layered onto the Gemini 3.6 Flash reasoning foundation](https://deepmind.google/models/model-cards/gemini-3-7-flash/), shipped just three weeks after 3.6. It reads text, images, audio, and video across a 1-million-token context window, and Google tuned this release for coding and agent work rather than raw reasoning: 65.3% on the DeepSWE v1.1 software engineering benchmark, up from 49.0% for the previous version, and 85.4% on LVBench, a video-understanding test. Pricing starts at $0.75 per million input tokens and $3.75 per million output tokens, an introductory rate that holds through December 31, 2026, before doubling on January 1, 2027.

DeepSeek's move came from a different direction. [DeepSeek V4 and V4-Pro shipped as the year's first open-weight frontier release back in April](https://docs.clore.ai/guides/language-models/deepseek-v4), and V4-Flash has been running at aggressive pricing ever since, roughly $0.14 per million input tokens on a cache miss, dropping to a fraction of a cent when the request hits cache. On August 21, DeepSeek extended that model with vision. [DeepSeek-V4-Flash-Vision-Exp matches the base V4-Flash on pure text tasks and brings a real jump on benchmarks that require visual understanding](https://emergent.sh/news/deepseek-v4-flash-vision-exp-officially), reportedly closing in on Anthropic's Opus-4.8 on agentic tasks that need both. DeepSeek hasn't published vision-specific pricing yet, but the text model it's built on already runs at a fraction of Gemini's rate.

Put the two side by side on comparable footing and the pattern holds. [Gemini 3.7 Flash wins 16 of 19 shared benchmarks against DeepSeek's equivalent Flash release, and it runs roughly twice as fast, about 12.65 tokens per second against DeepSeek's 5.9](https://www.orcarouter.ai/blog/gemini-3-7-flash-vs-deepseek-v4-flash/). DeepSeek answers with price: at current rates, it costs roughly 61% less per million tokens than Gemini. Speed and benchmark leadership on one side, a price nearly a third of the competitor's on the other. Neither company is trying to win the same argument.
